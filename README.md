...
CI runs [executor.yml](.github/workflows/executor.yml) every Monday and Thursday at 06:23 UTC (early, so proposals exist before the 14:40 UTC live routine), **propose-only** (with `EXECUTOR_KILL=1` as a hard stop), so you get the proposal card weekly without any execution risk.
